...
trait EncoderInstances:
  ...
  given Encoder[String] =
    Json.Str(_) // TODO Implement the `Encoder[String]` instance
...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(items) =>
        val inner: List[Option[A]] = items.map(decoder.decode(_))
        if inner.forall(_.isDefined) 
        then Some(inner.map((o: Option[A]) => o.get)) 
        else None
      case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(fields) => 
        ...
        then decoder.decode(fields(name))
        else None
      case _ => None
    }
