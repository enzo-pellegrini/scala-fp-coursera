...
abstract class TweetSet extends TweetSetInterface:
  ...
  def mostRetweetedAcc(soFar: Tweet): Tweet
...
class Empty extends TweetSet:
  ...
  def mostRetweetedAcc(soFar: Tweet): Tweet = soFar
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    ...
    val filteredLeft = left.filterAcc(p, newAcc)
  ...
  private def maxTweet(a: Tweet, b: Tweet): Tweet =
  ...
  def descendingByRetweet: TweetList =
    val biggest = mostRetweeted
    Cons(biggest, this.remove(biggest).descendingByRetweet)
...
object GoogleVsApple:
  ...
  def containsOneOfKeywords(ks: List[String])(t: Tweet): Boolean =
    ks.exists(kw => t.text.contains(kw))

  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter(containsOneOfKeywords(google))
  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter(containsOneOfKeywords(apple))
